...
namespace Magestore\Auction\Plugin\Category\Model;
...
class Layer
{
     ...
     */
    public function afterGetProductCollection(\Magento\Catalog\Model\Layer $object, $collection)
    {
        $auction = $this->_coreRegistry->registry('current_auction');
        if(count($auction)){
            $collection->addAttributeToFilter('entity_id',['in' => array_keys($auction)]);
        }
        return $collection;
    }
